...
# Must match path used in simulator.py
		import os
...
# Feed all received signals into the live plotter
				# Mirrors: self.plotter.update_data(t, x_current, x_desired, angle, force)
				# self.plotter.update_data(
				# 	vsiCommonPythonApi.getSimulationTimeInNs() * 1e-9,
				# 	self.mySignals.x_pos,
				# 	self.mySignals.y_pos,
				# 	self.mySignals.theta,
				# 	self.mySignals.lateral_error,
				# 	self.mySignals.heading_error,
				# 	self.mySignals.x_ref,
				# 	self.mySignals.y_ref,
				...
				self.mySignals.x_ref, receivedData = self.unpackBytes('d', receivedData, self.mySignals.x_ref)

				signalNumBytes = 8
				receivedData = vsiCanPythonGateway.recvVariableFromCanPacket(signalNumBytes, 0, 64, 18)
				self.mySignals.y_ref, receivedData = self.unpackBytes('d', receivedData, self.mySignals.y_ref)
